...
void filter_impl_cu(uint8_t* pixels_buffer, int width, int height, int plane_stride)
{
	...
	ImageView<float> distance_view = current_distance_pixels;


    background_process_frame(lab_image, current_background, candidate_background, current_time_pixels, distance_view);
