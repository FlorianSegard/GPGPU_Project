...
void red_mask_cpp(ImageView<bool> hysteresis_buffer, ImageView<rgb8> rgb_buffer, int width, int height, std::ptrdiff_t stride)
{
    return;
}
...
void mask_process_frame(ImageView<bool> hysteresis_buffer, ImageView<rgb8> rgb_buffer, int width, int height, std::ptrdiff_t stride)
{
    if (b_params.device == e_device_t::CPU)
        red_mask_cpp(hysteresis_buffer, rgb_buffer, width, height, stride);

    else if (b_params.device == e_device_t::GPU)
        red_mask_cu(hysteresis_buffer, rgb_buffer, width, height, stride);
