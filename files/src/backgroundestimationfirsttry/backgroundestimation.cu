...
__global__ void check_background(lab* in_buffer, std::ptrdiff_t stride_in,
...
__global__ void initialize_buffers(lab* zero_image_buffer, int* currentTimePixels_buffer, int width, int height, std::ptrdiff_t stride_image, std::ptrdiff_t stride_time) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
